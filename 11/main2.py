...
class Hall:
    ...
    def check_recursively(self, row, col, row_length, col_length, row_trav, col_trav):
        while True:
            if row_trav == -1 and row <= 0:
                return False
            if row_trav == 1 and row >= row_length:
                return False
            if col_trav == -1 and col <= 0:
                return False
            if col_trav == 1 and col >= col_length:
                return False
    ...
    def check_adjacent(self, seat):
        ...
        # check up
        if self.check_recursively(row, col, row_length, col_length, -1,0):
            full_adjacent += 1

        # check up_right
        if self.check_recursively(row, col, row_length, col_length, -1,1):
            full_adjacent += 1

        # check right
        if self.check_recursively(row, col, row_length, col_length, 0,1):
            full_adjacent += 1
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for seat in hall:
            ...
            elif taken >= 5 and not seat.isEmpty:
